// date.js —— 约会大作战App，从 keephone 项目移植到 liya
// 场景生成 / 约会剧情推进 / BGM播放器 / 付款机制(AA制、找角色借钱) / 立绘编辑器 / 预设管理 / 历史记录 / 结局分享
// 移植时改动：
//   1. generatePollinationsImage 从原项目 index.html 里提取过来，原样保留
//   2. updateUserBalanceAndLogTransaction / updateCharacterPhoneBankBalance 见下方"约会花销记账适配层"说明
...
// ---------------------------------------------------------------------------
// 约会花销记账适配层：keephone 原版用的是它自己独立的一套钱包字段
// (state.globalSettings.userBalance + db.userWalletTransactions)，这里改成
// 复用 liya 自己已有的支付宝钱包系统(data-management.js 里的 processTransaction，
// 对应 db.userWallet + window.userBalance + db.userTransactions)，这样约会花的钱
// 会正常出现在用户自己的支付宝账单里，而不是搞出第二套互不相通的"钱"。
// updateCharacterPhoneBankBalance 存的是角色自己手机里的零花钱(chat.characterPhoneData.bank)，
// 跟用户钱包是两回事，liya 原来没有对应实现，这个按原样搬过来。
// ---------------------------------------------------------------------------
async function updateUserBalanceAndLogTransaction(amount, description) {
  if (isNaN(amount)) return; // 安全检查
  if (amount >= 0) {
    await processTransaction(amount, 'income', description);
  } else {
    await processTransaction(-amount, 'expense', description);
  }
}
...
async function updateCharacterPhoneBankBalance(charId, amount, description) {
  ...
  console.log(
    `✅ 角色[${chat.name}]钱包已更新: 金额=${amount.toFixed(2)}, 新余额=${chat.characterPhoneData.bank.balance.toFixed(2)}`
  );
}
