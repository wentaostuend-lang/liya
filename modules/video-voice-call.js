...
`;
    ...
    if (videoCallState.isGroupCall) {
      ...
      inCallPrompt = `
        ...
        4.  **角色扮演**: 严格遵守每个角色的设定。
        5.  **【首句触发说明】**: 通话记录的第一条"用户"消息只是接通电话瞬间的画面提示(不是对方真正说的话，也不是在问你问题)。请不要把它当成需要被动回应的提问或问候，而是主动开口说出接通后你会说的第一句话（可以是打招呼、可以直接接续通话前摘要里的话题）。
        # 当前情景
        你们正在一个群视频通话中。
      ...
      inCallPrompt = `
        ...
        ${layoutRule}
        6.  **【首句触发说明】**: 通话记录的第一条"用户"消息只是接通电话瞬间的画面提示(不是对方真正说的话，也不是在问你问题)。请不要把它当成需要被动回应的提问或问候，而是主动开口说出接通后你会说的第一句话（可以是打招呼、可以直接接续通话前摘要里的话题）。
        # 当前情景
...
4. **角色扮演**: 严格遵守每个角色的设定。
5. **【首句触发说明】**: 通话记录的第一条"用户"消息只是电话接通瞬间的信号提示(不是对方真正说的话)。请不要把它当成一句需要被动回应的问候或提问，而是主动开口说出接通后你会说的第一句话。
...
5. **【首句触发说明】**: 通话记录里第一条"用户"消息（如"喂？"）只是电话接通瞬间的信号音提示，不是对方真的先开口说了这句话，更不是在问你"你是谁"。请把它当作"电话接通了"这个事件本身，然后主动说出你此刻会说的第一句话（可以打招呼，也可以直接接续通话前摘要里的话题），不要顺着它去确认对方身份或反问"是哪位"。
